...
class WP_Contributions_Plugins_Widget extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname'   => 'contributions_plugins',
			'description' => __( 'Add a list of your plugins on WordPress.org as a sidebar widget.', 'wp-contributions' )
		);

		parent::__construct( false, __( 'WP Contributions: Plugins', 'wp-contributions' ), $widget_ops );
	}
	...
	function form( $instance ) {
		if ( $instance && isset( $instance[ 'title' ] ) ) {
			...
			$title = esc_attr__( 'WP Plugins', 'wp-contributions' );
		}
		...
		if ( $instance && isset( $instance[ 'username' ] ) ) {
			...
			$username = esc_attr__( 'WordPress Username', 'wp-contributions' );
		}
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e( 'WordPress Username:', 'wp-contributions' ); ?></label>
	...
	function widget( $args, $instance ){
		...
		// Widget content
		$theme_args = array(
			'items' => array_slice( WP_Contributions_WordPress_Api::get_plugins( $user ), 0, $count ),
		);

		// Include template
		WP_Contributions::load_template( 'plugins-widget.php', $theme_args );
